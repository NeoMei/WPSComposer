...
M5 CROSS-PLATFORM ACCEPTANCE COMPLETE: Windows post-acceptance commits 45a178f and 7ffbd5b record 2517 passing platform-independent tests and three consecutive real WPS M5 gates (115.7/113.5/116.9 seconds) on WPS Office 12.1.0.28043. macOS accepted the Windows installer ignore fix and then passed 2545 tests with 12 explicit native/platform skips after adding reportlab to the dev test dependency contract and closing stale cross-platform status documentation. Version 0.8.0 remains unreleased; version bump, merge, and publication are separate release work.
WPSComposer 0.8.0 RELEASE: release metadata, public documentation, package version, and plugin manifest synchronized for the accepted M5 long-form engine. Publication target is GitHub master plus annotated tag and GitHub Release `v0.8.0`; post-merge verification is required before the tag is published.
