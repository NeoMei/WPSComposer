...
from ..artifact_transport import publish_artifact, validate_office_package, validate_pdf, copy_file_before_deadline, validate_before_deadline, ValidatorSpec
...
'''
        ...
        try:
            ...
            if not isinstance(envelope,list) or len(envelope)!=2 or envelope[0]!='WPSCOMPOSER_PPT_SESSION_OK':
                raise NativeOfficeError('NATIVE_OFFICE_EXECUTION_FAILED',staging_path=self._job,diagnostic_path=path.with_suffix('.stdout'))
            return str(envelope[1])
        except BaseException as exc:
            if isinstance(exc,subprocess.TimeoutExpired) or mutation:
            ...
            if isinstance(exc,subprocess.TimeoutExpired):
                ...
                path.with_suffix('.stderr').write_bytes(exc.stderr if isinstance(exc.stderr,bytes) else (exc.stderr or '').encode())
                raise NativeOfficeError('NATIVE_OFFICE_TIMEOUT',staging_path=self._job,diagnostic_path=path.with_suffix('.stderr')) from None
            raise
...
if sourceIndex < destinationIndex then
 ...
 move {obj} to before slide destinationIndex of ownedDoc
    ...
    def save(self,path):
        ...
        return str(publish_artifact(Path(self._path),target,overwrite=False,validator=lambda p:self._validate_artifact(p,'pptx'),deadline=self._deadline))
    ...
    def save_current(self):
        ...
        if self._attached:
            ...
            self._verify_source_digest()
            publish_artifact(Path(self._path),self._source,overwrite=True,validator=lambda p:self._validate_artifact(p,'pptx'),deadline=self._deadline)
            self._source_digest=self._digest(self._source)
