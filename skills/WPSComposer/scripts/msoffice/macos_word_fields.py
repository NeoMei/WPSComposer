...
def snapshot_commands(session):
    ...
    for handle,code in list(getattr(session,'_tracked_indexes',[])) + list(getattr(session,'_tracked_references',[])):
...
def snapshot(session):
    ...
    tracked=list(getattr(session,'_tracked_indexes',[]))+list(getattr(session,'_tracked_references',[]));by_position={}
    for handle,code in tracked:
        if handle.bookmark not in identities:_error(session)
        matches=[r for r in fields if r[3]==('REF' if handle.kind=='REF' else 'INDEX') and r[5]==identities[handle.bookmark] and r[4]==code]
