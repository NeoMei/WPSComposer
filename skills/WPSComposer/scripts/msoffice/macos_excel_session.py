...
return "{{}}"''',bind=False)
    ...
    def _quarantine(self,detail):
        ...
        if self._lock is not None:
            self._persist_diagnostic('quarantine',lambda:self._lock.quarantine(detail))

    def _write_logs(self,stem,stdout,stderr):
        first_error=None
    ...
    def _run(self,body,*,bind=True,acknowledge=None):
        ...
        if remaining<=0:
            ...
            raise RuntimeError('Excel session deadline expired; recovery required')
        self._counter+=1
...
if full name of ownedBook is not {_quote(str(self._native))} then error "Owned workbook identity mismatch"''' if bind else ''
        ...
        try:
            ...
            error=RuntimeError('Native Excel session failed; recovery required')
            error.staging_path=str(self._job)
            error.diagnostic_path=str(self._job/'recovery.json') if self._recovery_written else None
            error.code=detail['code']
            error.diagnostic_io_failures=tuple(getattr(self,'_diagnostic_io_failures',()))
            if getattr(self,'_clipboard_operation',False):error.clipboard_changed=True;error.clipboard_may_have_changed=True
            raise error from None
        log_error=self._write_logs(stem,stdout,stderr)
        if log_error is not None:
            detail={'component':'spreadsheet','staging_path':str(self._job),'owned_path':str(self._native),'step':self._counter,'closed':False,'code':'NATIVE_OFFICE_EXECUTION_FAILED'}
            if getattr(self,'_clipboard_operation',False):detail.update(clipboard_changed=True,clipboard_may_have_changed=True)
            self._quarantine(detail)
            raise log_error
        return output
...
if removedSheetName is in (name of every worksheet of ownedBook) then error "Deleted worksheet still present"'''
    ...
    def save_current(self):
        ...
        self._run('save ownedBook\nreturn '+_object({'saved':'saved of ownedBook'}),acknowledge='saved')
        if self._attached:
        ...
        published_state=snapshot_artifact_state(self._native,deadline=self._deadline)
        publish_artifact(
        ...
        if current_state.sha256 != published_state.sha256:
            raise RuntimeError('Logical Excel destination changed after publication')
        self._logical_state=current_state
        return str(self._logical)

    def save(self,path=None,*,destination=None):
        if path is not None and destination is not None:
            raise TypeError('Specify either path or destination, not both')
        path = destination if path is None else path
        if path is None:
            raise TypeError('save() missing required argument: path')
        self._assert_live()
        if self._attached:raise NotImplementedError('Attached Excel save-copy cannot preserve native binding')
        destination=self._destination(path,'.xlsx')
        self._run('save ownedBook\nreturn '+_object({'saved':'saved of ownedBook'}),acknowledge='saved')
        validate_before_deadline(ValidatorSpec.from_callable(validate_office_package,'xlsx'),self._native,self._deadline)
        published_state=snapshot_artifact_state(self._native,deadline=self._deadline)
    ...
    def save_copy(self,path=None,*,destination=None):
        if path is not None and destination is not None:
            raise TypeError('Specify either path or destination, not both')
        path = destination if path is None else path
        if path is None:
            raise TypeError('save_copy() missing required argument: path')
    ...
    def export_pdf(self,path=None,*,destination=None):
        if path is not None and destination is not None:
            raise TypeError('Specify either path or destination, not both')
        path = destination if path is None else path
        if path is None:
            raise TypeError('export_pdf() missing required argument: path')
        self._assert_live()
        if self._attached:raise NotImplementedError('Attached Excel export has no verified non-rebinding primitive')
        destination=self._destination(path,'.pdf')
    ...
    def close(self,save_changes=False):
        ...
        if not self._failed and not self._attached and save_changes:
            # A publication conflict is recoverable. Keep the owned workbook and
            # exclusive lock live so the caller can retry, save elsewhere, or
            # explicitly discard it with close(save_changes=False).
            self.save_current()
        try:
