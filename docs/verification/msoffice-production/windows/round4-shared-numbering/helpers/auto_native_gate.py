...
root = Path('build/msoffice-production/auto-01').resolve(); root.mkdir(exist_ok=False)
...
def processes():
    import psutil
    return sorted(p.pid for p in psutil.process_iter(['name']) if (p.info['name'] or '').lower() in {'winword.exe', 'wps.exe'})
try:
    before = processes()
