...
root=Path('build/msoffice-production/native-edit-02').resolve();root.mkdir(exist_ok=False)
...
def heading(text):
    r=c.doc.Content.Duplicate
    while r.Find.Execute(FindText=text, MatchCase=True, MatchWholeWord=False, Forward=True, Wrap=0):
        p=r.Paragraphs(1)
        if p.Range.Text.strip()==text and int(p.OutlineLevel)<=6:
            return p
        start=int(r.End)
        r=c.doc.Range(start,int(c.doc.Content.End))
    raise AssertionError('Heading not found: '+text)
