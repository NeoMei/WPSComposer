...
root=Path('build/msoffice-production/representative-01')
...
with fitz.open(root/'converted.pdf') as pdf:
    ...
    checks['toc_page_numbers_match_actual_body_page']=all(re.search(re.escape(h)+r'\.+3',compact[1]) and h in compact[2] for h in HEADINGS[:3])
...
report={'checks':checks,'all_passed':all(checks.values()),'body_indent_pt':indent,'pdf_heading_sizes_pt':sizes,'pdf_heading_size_tolerance_pt':.15,'rendered_pages':5,'visual_review':'All 5 pages inspected in contact sheet. No clipping or overlapping columns. Intentional cover/TOC pages and sparse final table page retained.'}
