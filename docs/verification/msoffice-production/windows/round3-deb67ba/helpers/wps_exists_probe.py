...
root=Path('build/msoffice-production/wps-exists-probe-01').resolve();root.mkdir(exist_ok=False)
...
try:
    ...
    for index in range(1,4):
        ...
        for kind in ['Headers','Footers']:
            ...
            row={'section':int(doc.Sections.Count),'part':kind,'initial_link':repr(part.LinkToPrevious)}
            try:
                row['initial_exists']=repr(part.Exists)
                part.Exists=True;row['exists_after_set']=repr(part.Exists)
                part.LinkToPrevious=False;row['link_after_exists']=repr(part.LinkToPrevious)
                part.Range.Text='';row['link_after_empty_text']=repr(part.LinkToPrevious)
                part.LinkToPrevious=False;row['link_after_empty_then_unlink']=repr(part.LinkToPrevious)
