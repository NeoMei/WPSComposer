...
root=Path('build/msoffice-production/sentinel-03').resolve();root.mkdir(exist_ok=False)
...
try:
    ...
    commands=[['representative-03','fixtures/verify_msoffice_production.py','--output-root','build/msoffice-production/representative-03','--fixture','representative','--engine','msoffice','--timeout','600']]
    report['runs']=[]
