...
def readback():
    return [
        ...
        '(style of followingRange is Word style (style body text) of boundDoc),'
...
def xml_checks(xml):
    ...
    return {'native_border_xml': (border is not None and border.get(W+'val') == 'single'
            ...
            'following_style_spacing_xml': (prop(following,'pStyle','val') == 'BodyText'
...
def artifacts(output):
    ...
    return dict(xml_checks(xml), pdf_text_visible='PARAGRAPH RULE' in text and 'FOLLOWING' in text,
                pdf_rule_drawn=any(d.get('color') and all(abs(c - 192/255) < 0.01 for c in d['color']) for d in drawings))
