...
def rollback(session,checkpoint):
    try:
        ...
        try:
            ...
            # Lightweight transports can return without crossing _execute's
            # submission boundary; a completed mutation still invalidates it.
            if session._field_topology_mutation_pending:session._invalidate_field_topology()
        finally:
