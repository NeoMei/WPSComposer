...
if sourceIndex < destinationIndex then
 ...
 move {obj} to before slide destinationIndex of ownedDoc
    ...
    def _semantic_slide(self,layout,lines):
        self._mutable()
        body='set index of document window 1 of ownedDoc to 1\nif full name of active presentation is not full name of ownedDoc then error "Owned presentation activation failed"\n'+f'set currentSlide to make new slide at end of ownedDoc with properties {{layout:{_enum(layout,LAYOUT)}}}\n'+'\n'.join(lines)+'\nset slide of view of document window 1 of ownedDoc to currentSlide\nreturn slide index of currentSlide'
