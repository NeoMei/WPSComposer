...
repeat with di from 1 to (count of presentations)
 if name of presentation di is "bound-a7a30fdaf9e64091ae1c469818db194a.pptx" then set identityCount to identityCount + 1
end repeat
if identityCount is not 1 then error "Stale or ambiguous bound presentation"
set ownedDoc to presentation "bound-a7a30fdaf9e64091ae1c469818db194a.pptx"
if full name of ownedDoc is not "/Users/neomei/Library/Containers/com.microsoft.Powerpoint/Data/Documents/wpscomposer/session-5oqi9ff7/bound-a7a30fdaf9e64091ae1c469818db194a.pptx" then error "Bound presentation path changed"
set results to {}
try
 activate document window 1 of ownedDoc
 set end of results to {"activate-window",true,full name of active presentation is full name of ownedDoc}
on error errorText number errorNumber
 set end of results to {"activate-window",false,errorNumber}
end try
try
 activate document window 1 of presentation "演示文稿68"
 set end of results to {"activate-sentinel",true,name of active presentation is "演示文稿68"}
on error errorText number errorNumber
 set end of results to {"activate-sentinel",false,errorNumber}
end try
try
 go to slide (view of document window 1 of ownedDoc) number 2
 set end of results to {"goto-owned",true,slide index of slide of view of document window 1 of ownedDoc,name of active presentation is "演示文稿68"}
on error errorText number errorNumber
