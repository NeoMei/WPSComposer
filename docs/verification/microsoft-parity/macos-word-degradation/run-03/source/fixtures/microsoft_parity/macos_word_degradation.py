...
def _effective_shading(run, paragraph, parents):
    """Resolve only this run's actual ancestors; explicit values stop inheritance."""
    candidates = [run.find(W+'rPr/'+W+'shd'), paragraph.find(W+'pPr/'+W+'shd')]
    ancestor = parents.get(paragraph)
    while ancestor is not None:
        if ancestor.tag == W+'tc':
            candidates.append(ancestor.find(W+'tcPr/'+W+'shd'))
        elif ancestor.tag == W+'tbl':
            candidates.append(ancestor.find(W+'tblPr/'+W+'shd'))
            break
        ancestor = parents.get(ancestor)
    for shading in candidates:
        if shading is not None:
            # val=clear with an explicit fill is normal solid shading. A clear
            # declaration without fill, nil, auto or another fill must not let
            # an ancestor's red background hide its explicit override.
            if shading.get(W+'val') == 'nil':
                return None
            return shading.get(W+'fill', 'auto').upper()
    return None
...
def inspect_ooxml(path, display, following, *, block, table):
    ...
    for run in notice.iter(W+'r'):
        ...
        if offset < end and right > start:
            ...
            assert _effective_shading(run, notice, parents) == 'FCE8E6', 'Notice shading missing'
    ...
    for run in normal[0].iter(W+'r'):
        assert _effective_shading(run, normal[0], parents) != 'FCE8E6', 'Following normal text inherited shading'
        props = run.find(W+'rPr')
        if props is None:
            continue
