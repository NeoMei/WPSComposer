...
C.CDLL('/System/Library/Frameworks/AppleScriptObjC.framework/AppleScriptObjC')
...
objc.objc_msgSend(objc.objc_getClass(b'NSApplication'),objc.sel_registerName(b'sharedApplication'))
bundle=objc.objc_msgSend(objc.objc_getClass(b'NSBundle'),objc.sel_registerName(b'mainBundle'))
objc.objc_msgSend(bundle,objc.sel_registerName(b'loadAppleScriptObjectiveCScripts'))
...
def send(event,reply,mode,priority,timeout,idle,filter_,unused):
 ...
 try:
  ...
  print('TARGET',address.kind.to_bytes(4,'big'),buf.raw.hex(),file=sys.stderr)
