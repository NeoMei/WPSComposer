...
class SourceOnlyListFixtureTest(unittest.TestCase):
    ...
    def test_native_readback_requires_all_list_and_following_body_formats(self):
        module = load_runner()
        good = [
            ["builtin-styles", "正文文本", "列表段落"],
            [1, "•\tDEFAULT ITEM\r", "列表段落", 24, -24],
            [2, "BODY AFTER DEFAULT\r", "正文文本", 0, 24],
            [3, "→\tCUSTOM ITEM\r", "列表段落", 30, -30],
            [4, "\tEMPTY GLYPH ITEM\r", "列表段落", 27.5, -27.5],
            [5, "1.\tORDERED ITEM\r", "列表段落", 33, -33],
            [6, "BODY AFTER ORDERED\r", "正文文本", 0, 24],
        ]
        ...
        arbitrary = [list(row) for row in good]
        arbitrary[3][2] = "任意样式"
        self.assertFalse(module.validate_readback(arbitrary))
