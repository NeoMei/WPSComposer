...
def main():
    ...
    out=root/'build/excel-command-boundary-20260910/private-json-roundtrip-05';out.mkdir(exist_ok=False)
    from skills.WPSComposer.scripts.msoffice import macos_osa_transport as osa
    helper=Path(osa.__file__); real_run=subprocess.run
    runtime=osa._DarwinRuntime(); pool=runtime.message(runtime.objc_class('NSAutoreleasePool'),'new')
    lookup=osa._DarwinProcessLookup(runtime)
    identities={}
    def bound(target):
     current=lookup.snapshot(int(target))
     if int(target) not in identities: identities[int(target)]=current
     osa.require_same_process(identities[int(target)],current)
     return osa.BoundOSAKitTransport(identities[int(target)],runner=real_run)
    ...
    def call(target,label,body):
     ...
     r=bound(target).run(script,time.monotonic()+30)
    ...
    try:
     ...
     with patch('subprocess.run',wrapped):
      ...
      session._run('set value of range "Z40" of worksheet 1 of ownedBook to "PRIVATE-JSON-ROUNDTRIP-05"\nreturn "{}"')
    ...
    return '''+_object({'deleted':'true'})
      ...
      assert got=={'names':['Data','Results'],'marker':'PRIVATE-JSON-ROUNDTRIP-05','alerts':True},got
