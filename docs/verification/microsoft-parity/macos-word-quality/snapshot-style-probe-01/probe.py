...
try:
    ...
    vectors={}
    for label,prop in [('names','name local'),('in-use','in use'),('built-in','built in'),('auto-update','automatically update')]:
    ...
    report['vector_lengths']={k:len(v) for k,v in vectors.items()};flush()
    safe=all(step['after'][1:3]==[True,True] and step['after'][6]==report['baseline'][6] for step in report['steps'])
    report['checks']['bulk_getters_stable']=safe
    if set(vectors)=={'names','in-use','built-in','auto-update'} and len(set(map(len,vectors.values())))==1 and all(type(x) is bool for k in ('in-use','built-in') for x in vectors[k]):
        defined=[name for name,used,builtin in zip(vectors['names'],vectors['in-use'],vectors['built-in']) if used or not builtin]
        report['defined_candidates']=defined;report['in_use_names']=[name for name,used in zip(vectors['names'],vectors['in-use']) if used];report['custom_names']=[name for name,builtin in zip(vectors['names'],vectors['built-in']) if not builtin];flush()
        if safe:
            for i,name in enumerate(defined):
                rows=read_block(f'defined-description-{i+1}',[
                    'set qstyle to Word style '+apple_string(name)+' of boundDoc',
                    'set probeDescription to description of qstyle as text',
                    'set probeRows to {{"description",'+apple_string(name)+',probeDescription}}'])
                step=report['steps'][-1]
                if rows[0][0]!='read-ok' or step['after'][1:3]!=[True,True] or step['after'][6]!=report['baseline'][6]:
                    safe=False;report['description_blocker']=name;break
        report['checks']['defined_descriptions_stable']=safe
        if safe:
            replacement=[
                ...
                'set qualityStyleAutoUpdate to get automatically update of every Word style of boundDoc',
                'if (count qualityStyleNames) is not (count qualityStyleInUse) or (count qualityStyleNames) is not (count qualityStyleBuiltIn) or (count qualityStyleNames) is not (count qualityStyleAutoUpdate) then error "WPSC_QUALITY_STYLE_IDENTITY_UNVERIFIED"',
                ...
                'set qualityStyleAuto to item qualityStyleOrdinal of qualityStyleAutoUpdate',
                'set end of qualityLayout to {"style-state",qualityStyleName,qualityStyleUsed,qualityStyleBuiltin,qualityStyleAuto}',
            ...
            step=report['steps'][-1]
            if report['checks']['complete_snapshot_valid'] and step['after'][1:3]==[True,True] and step['after'][6]==report['baseline'][6]:
                second=read_block('complete-snapshot-repeat',revised,'state')
                report['checks']['repeat_snapshot_equal']=second==full
    else:report['blocker']='Style classification unavailable or malformed; no definition filtering performed.'
