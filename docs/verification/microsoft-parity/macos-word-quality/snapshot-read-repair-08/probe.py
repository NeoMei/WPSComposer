...
try:
    ...
    for position in (0,59,110):
        lines=[f'set probeRange to create range boundDoc start {position} end {position+1}',
            ...
            'set probeRows to {{"same-coordinate",'+str(position)+',probeEqual as boolean,probeLegacyRow,probeRecordRow}}']
        blocks.append(('record-vs-legacy-'+str(position),lines))
    try:
