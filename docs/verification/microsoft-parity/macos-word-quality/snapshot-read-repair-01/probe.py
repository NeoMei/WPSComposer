...
try:
    ...
    blocks.append(('prefix-suffix-hashes', snapshot[:snapshot.index('set qualityLayout to {}')]))
    blocks.append(('section-object', ['set qsection to section 1 of boundDoc','set probeRows to {{"section-reference-resolved"}}']))
    blocks.append(('section-setup-object', ['set qsection to section 1 of boundDoc','set qsetup to page setup of qsection','set probeRows to {{"setup-reference-resolved"}}']))
    setup=[x for x in q._layout_commands() if x.startswith('set end of qualityLayout to {"section"')][0]
    blocks.append(('section-setup-values',['set qi to 1','set qsection to section 1 of boundDoc','set qsetup to page setup of qsection',setup,'set probeRows to qualityLayout']))
    for variant in ('primary','first page','even pages'):
        for part in ('header','footer'):
            setup=['set qsection to section 1 of boundDoc',f'set qpart to get {part} qsection index header footer {variant}']
            prefix=variant.replace(' ','-')+'-'+part
            blocks.append((prefix+'-get-object',setup+['set probeRows to {{"part-reference-resolved"}}']))
            blocks.append((prefix+'-count-shapes',setup+['set probeRows to {{"shapes",(count shapes of qpart)}}']))
            blocks.append((prefix+'-text-object',setup+['set qr to text object of qpart','set probeRows to {{"text-reference-resolved"}}']))
            blocks.append((prefix+'-text',setup+['set qr to text object of qpart',*hash_commands('content of qr as text','probePartHash'),'set probeRows to {{"text-hash",probePartHash}}']))
            for label,prop in [('index','header footer index of qpart as text'),('is-header','is header of qpart'),('link-to-previous','link to previous of qpart')]:
                blocks.append((prefix+'-'+label,setup+['set probeRows to {{"metadata",'+prop+'}}']))
            blocks.append((prefix+'-fields-count',setup+['set qr to text object of qpart','set probeRows to {{"fields",(count fields of qr)}}']))
    try:
