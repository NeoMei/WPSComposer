...
try:
    ...
    blocks=[]
    blocks.append(('story-count',['set probeRows to {{"story-count",(count story ranges of boundDoc)}}']))
    blocks.append(('story-types',['set probeTypes to get story type of every story range of boundDoc','set probeRows to {{"story-types",probeTypes}}']))
    ...
    for variant in ('primary','first page','even pages'):
        for part in ('header','footer'):
            prefix=variant.replace(' ','-')+'-'+part
            setup=[f'set qr to get story range boundDoc story type {variant} {part} story']
            blocks.append((prefix+'-get-story',setup+['set probeRows to {{"story-reference-resolved"}}']))
            blocks.append((prefix+'-story-hash',setup+[*hash_commands('content of qr as text','probePartHash'),'set probeRows to {{"story-hash",probePartHash,(count fields of qr)}}']))
    try:
