...
try:
    ...
    # Range type/Start/End were extra prototype observations, absent from original pagepart coverage.
    extra='story type of probeStoryRange as text,start of content of probeStoryRange as integer,end of content of probeStoryRange as integer,'
    story_lines=[line.replace(extra,'') for line in story_lines]
    probe_lines=[line.replace(extra,'') for line in probe_lines]
    (OUT/'full-candidate-lines.json').write_text(json.dumps(probe_lines,indent=2))
    try:
        for prop,expression in [('type','story type of probeStoryRange as text'),('start','start of content of probeStoryRange as integer'),('end','end of content of probeStoryRange as integer'),('content','content of probeStoryRange as text'),('field-count','(count fields of probeStoryRange) as integer')]:
            read_block('primary-header-scalar-'+prop,['set probeStoryRange to missing value','set probeStoryRange to get story range boundDoc story type primary header story','set probeRows to {{"value",'+expression+'}}'])
