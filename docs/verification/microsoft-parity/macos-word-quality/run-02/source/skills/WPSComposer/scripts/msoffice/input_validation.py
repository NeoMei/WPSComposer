...
def _validate_package(path, component, deadline):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        for info in infos:
            _check_deadline(deadline)
            # ZipInfo may normalize Windows separators or truncate NULs before
            # exposing filename. Validate the original archive spelling first.
            raw_name = info.orig_filename
            _part_name(raw_name[:-1] if raw_name.endswith('/') else raw_name)
            if info.is_dir():
