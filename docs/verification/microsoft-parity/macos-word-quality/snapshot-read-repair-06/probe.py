...
try:
    ...
    blocks=[]
    for story in ('main text','primary header'):
        for mode in ('original','evaluated'):
            setup=['set probeStoryRange to missing value','set probeStoryRange to get story range boundDoc story type '+story+' story']
            if mode=='evaluated':setup+=['set probeStoryRange to get probeStoryRange']
            prefix=story.replace(' ','-')+'-'+mode
            blocks.append((prefix+'-class',setup+['set probeRows to {{"class",(class of probeStoryRange) as text}}']))
            for label,expression,cast in [('type','story type of probeStoryRange','as text'),('start','start of content of probeStoryRange','as integer'),('end','end of content of probeStoryRange','as integer'),('content','content of probeStoryRange','as text')]:
                lines=setup+['set probeValue to "unresolved-sentinel"','set probeValue to get '+expression,'set probeValueClass to (class of probeValue) as text']
                if label=='content':lines+=[*hash_commands('probeValue as text','probeValueHash'),'set probeRows to {{"value-hash",probeValueClass,probeValueHash}}']
                else:lines+=['set probeRows to {{"scalar",probeValueClass,probeValue '+cast+'}}']
                blocks.append((prefix+'-'+label,lines))
    try:
