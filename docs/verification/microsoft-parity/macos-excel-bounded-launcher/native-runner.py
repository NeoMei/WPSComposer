...
def main():
 ...
 files=[ROOT/'skills/WPSComposer/scripts/msoffice'/n for n in ['macos_excel_session.py','macos_excel_process.py','macos_osa_transport.py','macos_office_runtime.py','macos_excel_launcher.py']]
 hashes={}
...
return '''+_object({'books':'probeRows','alerts':'display alerts'})+'\nend tell')
 ...
 try:
  ...
  for phase,path in [('edit',source),('reopen',out/'result.xlsx')]:
   ...
   launcher=session._process_owner._launcher
   helper=launcher.process
   assert helper is not None and helper.poll() is None
   report.setdefault('helpers',[]).append({'pid':helper.pid,'class':type(launcher).__name__})
   assert type(launcher).__name__=='BoundedExcelLauncher'
   ...
   if phase=='edit':
    ...
    pdf_path=out/'result.pdf'
    assert pdf_path.read_bytes().startswith(b'%PDF-')
    report['pdf_pages']=len(PdfReader(pdf_path).pages)
    assert report['pdf_pages']>0
   else:
   ...
   while lookup.snapshot(ident.pid) is not None and time.monotonic()<end:time.sleep(.1)
   assert lookup.snapshot(ident.pid) is None
   helper.wait(timeout=2)
   report['helpers'][-1]['exitcode']=helper.returncode
