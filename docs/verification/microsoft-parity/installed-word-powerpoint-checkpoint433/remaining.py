...
p=root/'build/installed-checkpoint433/remaining';p.mkdir(exist_ok=False)
...
for kind in ('writer','slide'):
 ...
 # These are smoke runs in an empty application; do not imply sentinel coverage.
...
report['sources_equal_after']=sources_equal();report['passed']=len(report['runs'])==2 and report['sources_equal_after'] and all(r['returncode']==0 and r['inventory_before']==r['inventory_after'] and r['report']['passed'] for r in report['runs'].values())
