...
def snapshot_commands(session):
    ...
    for handle,code in getattr(session,'_tracked_indexes',[]):
...
def snapshot(session):
    ...
    for row in rows[1:]:
        ...
        elif len(row)==9 and row[0]=='field' and all(isinstance(row[i],str) for i in (1,3,4,6)) and all(type(row[i]) is int for i in (2,5,7,8)) and row[2]>0 and row[5]>=0 and row[7]>=(1 if row[3]=='INDEX' else 0) and row[8]>=row[7]:fields.append(row)
        else:_error(session,'NATIVE_WORD_EXECUTION_FAILED')
    tracked=getattr(session,'_tracked_indexes',[]);by_position={}
    for handle,code in tracked:
        if handle.bookmark not in identities:_error(session)
        matches=[r for r in fields if r[3]=='INDEX' and r[5]==identities[handle.bookmark] and r[4]==code]
        if len(matches)!=1:_error(session)
        by_position[(matches[0][1],matches[0][2])]=handle
    counts={'TOC':0,'TOF_FIG':0,'TOF_TAB':0};resolved=[];observed=[]
    for row in fields:
        _,owner,index,kind,code,start,visible,first,last=row
        handle=by_position.get((owner,index))
        if handle:owner,kind,category=handle.owner_node_id,handle.kind,'index'
        elif kind=='INDEX':continue  # shared baseline tracks only inserted indexes
        else:
            category='page' if kind in {'PAGE','NUMPAGES'} else 'field'
            observed.append((owner,index,kind,hashlib.sha256(code.encode()).hexdigest(),start,len(visible.encode('utf-16-le'))//2))
        if kind in counts:counts[kind]+=last-first+1
        resolved.append((owner,kind,category,visible))
    ...
    ordinals={};snapshots=[]
    for owner,kind,category,visible in resolved:
        key=(owner,kind);ordinal=ordinals.get(key,0);ordinals[key]=ordinal+1
