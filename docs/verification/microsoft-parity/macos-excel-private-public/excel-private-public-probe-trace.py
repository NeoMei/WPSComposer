...
def main():
 ...
 original=None
 original_snapshot=osa._DarwinProcessLookup.snapshot
 trace=[]
 def traced_snapshot(lookup,pid):
  entry={'pid':pid,'at':time.monotonic(),'calls':[]};trace.append(entry)
  runtime=lookup.runtime;original_message=runtime.message
  original_info=runtime.proc_pidinfo;original_path=runtime.proc_pidpath
  def info(*a):
   value=original_info(*a);entry['calls'].append({'proc_pidinfo_result':value});return value
  def path(*a):
   value=original_path(*a);entry['calls'].append({'proc_pidpath_result':value});return value
  runtime.proc_pidinfo=info;runtime.proc_pidpath=path
  def message(*args,**kwargs):
   value=original_message(*args,**kwargs)
   entry['calls'].append({'selector':args[1],'result':value if isinstance(value,(int,float,str,bool)) or value is None else str(type(value))})
   return value
  runtime.message=message
  try:
   value=original_snapshot(lookup,pid);entry['identity']=asdict(value) if value else None;return value
  except BaseException as e:entry['error']=str(e);raise
  finally:
   runtime.message=original_message;runtime.proc_pidinfo=original_info;runtime.proc_pidpath=original_path
   (out/'lookup-trace.json').write_text(json.dumps(trace,indent=2))
 osa._DarwinProcessLookup.snapshot=traced_snapshot
