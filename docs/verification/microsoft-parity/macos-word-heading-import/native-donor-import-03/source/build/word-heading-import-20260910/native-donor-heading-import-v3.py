"""V3 bounded donor import acceptance with same-text native style controls.
...
SOURCES=[Path(__file__),Path(__file__).with_name('test_native_donor_heading_import_v3.py'),
...
def control_signatures(xml,source_id,clone_id):
    ...
    for index,identifier in [(1,source_id),(4,clone_id)]:
        ...
        if ''.join(t.text or '' for t in p.iter(W+'t'))!=FRESH:raise ValueError('Wrong indexed control text')
        ...
        if len(styles)!=1 or styles[0].attrib!={W+'val':identifier}:raise ValueError('Wrong control style identity')
        if any(c.tag not in (W+'pPr',W+'r') for c in p) or any(x.tag not in (W+'rPr',W+'t') for r in p.findall(W+'r') for x in r):raise ValueError('Unsupported control content')
        # Full pPr except the intentionally distinct identity; full run content,
        # order and direct properties. No rFonts/hint/szCs normalization.
        result.append((tuple(canonical(c) for c in props[0] if c.tag!=W+'pStyle'),
                       tuple((''.join(t.text or '' for t in c.findall(W+'t')),tuple(canonical(r) for r in c.findall(W+'rPr'))) for c in p if c.tag==W+'r')))
    return tuple(result)
...
def source_only_challenge_xml(before,after):
    ...
    source=paragraph_style_by_name(expected.findall(W+'style'),'heading 1')
    if source is None:return False
    size=source.find('./'+W+'rPr/'+W+'sz');spacing=source.find('./'+W+'pPr/'+W+'spacing')
    if size is None or spacing is None or spacing.get(W+'before') is None:return False
    size.set(W+'val',str(int(size.get(W+'val'))+22))
    spacing.set(W+'before',str(int(spacing.get(W+'before'))+140))
...
def run(output):
    ...
    try:
        ...
        with MacWordSession.new_document(visible=False) as session:
            ...
            check('only_exact_source_style_challenge',source_only_challenge_xml(baseline_styles,challenged_styles))
