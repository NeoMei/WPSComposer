"""Continue exact saved italic-off artifact; export then restore source italic only.
...
BASELINE=BASE/'native-donor-import-05/heading.docx'
BASELINE_SHA='1655223f01b9e500790c2f8dc3fcba0960628e46185da0a9e57f27ec4bd23071'
INPUT=BASE/'italic-face-01/italic-off.docx'
INPUT_SHA='94292aea7ee7471072a450a7de73a884f5989a582178f408f92cb74ce730371e'
...
def italic_off_valid(before,after):
    """Exact observed deletion, with pinned nonitalic inheritance prerequisites."""
    try:
        expected=ET.fromstring(before);originals=paired_styles(expected)
        index={s.get(W+'styleId'):s for s in expected.findall(W+'style')}
        if expected.findall('./'+W+'docDefaults/'+W+'rPrDefault/'+W+'rPr/'+W+'i'):return False
        for original,base_id in zip(originals,('a','a0')):
            bases=original.findall(W+'basedOn')
            if len(bases)!=1 or bases[0].attrib!={W+'val':base_id}:return False
            base=index.get(base_id)
            if base is None or base.findall(W+'basedOn') or base.findall('./'+W+'rPr/'+W+'i'):return False
            old=original.findall('./'+W+'rPr/'+W+'i')
            if len(old)!=1 or old[0].attrib or len(old[0]) or old[0].text or old[0].tail:return False
            original.find(W+'rPr').remove(old[0])
...
def pdf_observations(path):
    """Geometric paragraph attribution; missing-glyph counts remain observations."""
    import fitz
    markers=[];characters=[]
            ...
            for block in page.get_text('dict')['blocks']:
                for line in block.get('lines',[]):
                    for span in line['spans']:
                        for marker in ('FRESH STYLE','IMPORTED APPEARANCE','SUFFIX'):
                            if span['text'].startswith(marker):markers.append(((page_index,span['origin'][1]),marker))
            for trace in page.get_texttrace():
                for char in trace['chars']:characters.append(((page_index,char[2][1]),char[0],char[1]))
            page.get_pixmap(matrix=fitz.Matrix(1.5,1.5)).save(path.with_name(path.stem+'-page-'+str(page_index+1)+'.png'))
    markers.sort()
    if [m[1] for m in markers]!=['FRESH STYLE','IMPORTED APPEARANCE','SUFFIX','FRESH STYLE']:raise ValueError('Unexpected PDF paragraph markers')
    groups={name:[] for name in ('source','imported','suffix','clone')}
    for position,scalar,gid in characters:
        eligible=[i for i,(start,_) in enumerate(markers) if start[0]<position[0] or (start[0]==position[0] and start[1]<=position[1]+1)]
        if not eligible:raise ValueError('Unattributed PDF text')
        groups[list(groups)[max(eligible)]].append((scalar,gid))
    return {name:{'characters':len(rows),'glyph_zero_count':sum(gid==0 for _,gid in rows),
                  'unicode_error_count':sum(scalar in (0,65533) for scalar,_ in rows),
                  'arabic_scalars':sum(0x600<=scalar<=0x6ff for scalar,_ in rows),
                  'raw_scalar_glyph_pairs':rows} for name,rows in groups.items()}


def run(output):
    sources=[Path(__file__),BASE/'test_italic_face_continuation.py',*v5.SOURCES]
    ...
    try:
        check('pinned_input',sha(INPUT)==INPUT_SHA);check('pinned_baseline',sha(BASELINE)==BASELINE_SHA)
        styles,baseline_xml=package(BASELINE);off_styles,xml=package(INPUT);signature=document_signature(baseline_xml)
        check('exact_off_deletions',italic_off_valid(styles,off_styles))
        check('off_document_preserved',document_signature(xml)==signature)
        ...
        check('off_state',valid_state(execute('off_state',state_commands()),body,False))
        owner.export_pdf(output/'italic-off.pdf')
        check('off_after_pdf_state',valid_state(execute('off_after_pdf',state_commands()),body,False))
        for label,value in [('restored',True)]:
        ...
        check('original_input_preserved',sha(INPUT)==INPUT_SHA and sha(BASELINE)==BASELINE_SHA);check('final_empty',v5.inventory(output,'final')==[])
        report['pdf_observations']={label:pdf_observations(output/(label+'.pdf')) for label in ('italic-off','restored')};flush()
...
def main():
    ...
    if args.compile_only:
        ...
        for label,lines in [('restore',toggle_commands(body,True)),('state',state_commands())]:
