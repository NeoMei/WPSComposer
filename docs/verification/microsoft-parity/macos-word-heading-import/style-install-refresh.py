...
def setup_commands():
    return ['set sourceStyle to Word style (style heading1) of boundDoc',f'set parentStyle to make new Word style at boundDoc with properties {{name local:{apple_string(PARENT)}}}','set base style of parentStyle to style normal','set paragraph format left indent of paragraph format of parentStyle to 17','set base style of sourceStyle to parentStyle',f'set headingRange to create range boundDoc start 0 end {units(v5.FRESH)}',f'make new bookmark at boundDoc with properties {{name:{apple_string(BOOKMARK)},text object:headingRange}}','set ownSelection to selection of boundWindow','set selection start of ownSelection to 0','set selection end of ownSelection to 5','set nativeRows to {{"setup",true}}']
...
def run(output):
    ...
    try:
        ...
        ack=execute('setup',setup_commands());owner.save_docx(output/'setup.docx');check('setup_ack',ack==[['setup',True]] and ack[0][1] is True)
...
def main():
    ...
    if a.compile_only:
        ...
        for label,lines in [('setup',setup_commands()),('state',state_commands()),('import',v5.collapsed.collapsed_commands(Path('/tmp/COMPILE_ONLY.docx'),start,body)),('clear',clear_commands(inserted,start,start+units(CARRIER+'\r')))]:
