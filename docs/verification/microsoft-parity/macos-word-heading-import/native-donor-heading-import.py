"""Owned-recipient heading import feasibility using a proven Word-saved donor.
...
SOURCES=[Path(__file__),Path(__file__).with_name('test_native_donor_heading_import.py'),
         Path(collapsed.__file__),Path(gate.__file__),*ORIGINAL_SOURCES]
...
def run(output):
    report = {'status': 'FAIL', 'scope': 'Owned native DOCX donor import with matching source/theme only; public method remains absent',
