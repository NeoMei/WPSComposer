...
@pytest.mark.parametrize('preimage,start,end,want', [
    ('A\rREPLACE\rTAIL\r', 2, 10, 'A\rNEW\rTAIL\r'),
    ('A\rREPLACE\rTAIL\r\r', 2, 10, 'A\rNEW\rTAIL\r\r'),
    ('😀\rREPLACE\r尾\r\r', 3, 11, '😀\rNEW\r尾\r\r'),
])
def test_replacement_preserves_actual_native_terminal_paragraphs(preimage, start, end, want):
    assert module().replacement_expectation(preimage, start, end, 'NEW\r') == want


@pytest.mark.parametrize('preimage,start,end', [
    ('A\rREPLACE\rTAIL\r', -1, 10),
    ('A\rREPLACE\rTAIL\r', 2, 99),
    ('A\rREPLACE\rTAIL\r', 10, 2),
    ('A\rREPLACE\rTAIL\r', True, 10),
    ('A\rREPLACE\rTAIL\r', 2.0, 10),
    ('😀\rREPLACE\rTAIL\r', 1, 11),
    ('A\rREPLACE\r😀\r', 2, 11),
    ('A\rCHANGED\rTAIL\r', 2, 10),
])
def test_replacement_rejects_invalid_utf16_bounds_or_changed_target(preimage, start, end):
    with pytest.raises(ValueError):
        module().replacement_expectation(preimage, start, end, 'NEW\r')
