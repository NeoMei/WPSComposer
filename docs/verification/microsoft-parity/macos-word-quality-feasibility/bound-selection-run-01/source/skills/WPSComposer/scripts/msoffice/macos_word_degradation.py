...
def _table_ack(rows, display, position):
    ...
    if not (isinstance(row, list) and len(row) == 14
            ...
            # Word expands both cell and row terminators as CR/BEL in Text,
            # but each consumes one native story coordinate (probe 16.112.3).
            and isinstance(row[4], str) and row[4] == display+'\r\x07\r\x07'
            and row[3] == row[2]+_units(display)+2
