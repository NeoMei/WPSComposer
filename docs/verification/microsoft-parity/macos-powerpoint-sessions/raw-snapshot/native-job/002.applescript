...
on normalizedJSON(v)
 if v is missing value then return current application's NSNull's null()
...
repeat with di from 1 to (count of presentations)
 if name of presentation di is "bound-fca412cee9c145949d427b2aa07d1cb0.pptx" then set identityCount to identityCount + 1
end repeat
if identityCount is not 1 then error "Stale or ambiguous bound presentation"
set ownedDoc to presentation "bound-fca412cee9c145949d427b2aa07d1cb0.pptx"
if full name of ownedDoc is not "/Users/neomei/Library/Containers/com.microsoft.Powerpoint/Data/Documents/wpscomposer/session-a_8gpm4r/bound-fca412cee9c145949d427b2aa07d1cb0.pptx" then error "Bound presentation path changed"
 ...
 repeat with sh from 1 to (count of shapes of sl)
  ...
  if has text frame of shp then
   ...
   repeat with pi from 1 to (count of paragraphs of tx)
    set end of snapshotRows to {"paragraph",si as integer,sh as integer,pi as integer,content of paragraph pi of tx}
   end repeat
   set pf to paragraph format of tx
   set tf to text frame of shp
   set ln to line format of shp
   set end of snapshotRows to {"detail",si as integer,sh as integer,fore color of ln,weight of ln,visible of ln,transparency of ln,margin left of tf,margin right of tf,margin top of tf,margin bottom of tf,word wrap of tf,alignment of pf as text,space before of pf,space after of pf,space within of pf}
  end if
