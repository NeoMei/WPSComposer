...
from ..artifact_transport import publish_artifact, copy_file_before_deadline, validate_office_package, validate_pdf, validate_before_deadline, ValidatorSpec
...
return rangeJSON''')
    ...
    def apply_structural_op(self,op):
        ...
        if verb in ('move','clone'):
            ...
            lines=[prefix,('cut' if verb=='move' else 'copy')+' range (entire '+axis+' of obj)',f'insert into range (range {_quote(destination)} of ws)','return '+_object({'type':_quote(axis),'moved' if verb=='move' else 'cloned':'true','from':_quote(target),axis:str(result_index),'clipboard_changed':'true','requested_index':str(dest)})]
            return self._run('\n'.join(lines))
...
return {_object({'type':'"sheet"','path':'"sheet:" & (count of worksheets of ownedBook)'})}'''
    ...
    def write_cell(self,row,col,value):
        return self.apply_format_patch(self._business_sheet()+'/cell:'+self._cell_address(row,col),value=value)
