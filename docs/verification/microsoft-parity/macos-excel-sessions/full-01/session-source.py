...
from ..artifact_transport import copy_file_before_deadline, validate_office_package, validate_pdf
...
return "{{}}"''',bind=False)
    ...
    def _run(self,body,*,bind=True):
        ...
        if remaining<=0: raise RuntimeError('Excel session deadline expired')
        self._counter+=1
    ...
    def apply_format_patch(self,target,**patch):
        ...
        for key,value in patch.items():
            ...
            if key in ('value','formula'): add(key,key+' of obj',_native_value(value))
                ...
                if not isinstance(value,dict) or set(value)-{'color'}: raise ValueError('Unsupported Excel fill fields')
                if not range_kind: raise NotImplementedError('Native Excel shape fill adapter is not yet verified')
                if 'color' in value: add('fill.color','color of interior object of obj',_color(value['color']))
            elif key=='line': raise NotImplementedError('Native Excel shape line adapter is not yet verified')
            elif key=='geometry':
...
return {_object({'type':'"sheet"','path':'"sheet:" & (count of worksheets of ownedBook)'})}'''
        ...
        if verb=='insert':
            ...
            lines=[f'set ws to worksheet {sheet} of ownedBook',f'insert into range (range {_quote(address)} of ws)']
    ...
    def save(self,destination):
        ...
        self._run('save ownedBook\nreturn "{}"')
        validate_office_package(self._native,'xlsx')
