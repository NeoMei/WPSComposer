...
def _matches_published_identity(expected: ArtifactState, target: Path) -> bool:
    """Check whether *target* is still the exact inode we just published.

    Rollback must remain available after the public deadline expires, so this
    deliberately uses one non-following stat instead of content I/O. A changed,
    deleted, or retargeted path is preserved and the prior backup is retained.
    """
    try:
        current = Path(target).lstat()
    except OSError:
        return False
    return (
        ...
        and current.st_mtime_ns == expected.modified_ns
    )
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            try:
                if not _matches_published_identity(published_state, target):
                    recovery_path = backup
                    backup = None
                    recovery = (
                        f"; previous artifact retained at {recovery_path}"
    ...
    *,
    ...
    try:
        ...
        for target in targets:
            try:
                current_state = snapshot_artifact_state(target, deadline=deadline)
            except TimeoutError:
                raise
            except (OSError, ValueError, ArtifactTransportError) as state_exc:
        ...
        for target in reversed(published):
            ...
            try:
                if (
                    ...
                    or not _matches_published_identity(published_states[target], target)
                ):
                    raise ArtifactTransportError(
                        "ARTIFACT_ROLLBACK_FAILED",
