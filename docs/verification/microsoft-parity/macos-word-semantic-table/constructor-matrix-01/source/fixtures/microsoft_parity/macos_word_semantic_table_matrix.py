"""Four bounded independent constructor observations, no production changes.
...
CASES=(('A-small-inactive',1,1,19,False),('B-collapsed-inactive',4,3,12,False),
       ('C-replacement-inactive',4,3,19,False),('D-replacement-active',4,3,19,True))
SOURCES=[*sorted((ROOT/'skills/WPSComposer').rglob('*.py')),*sorted((ROOT/'fixtures/microsoft_parity').glob('*.py'))]


def commands(path,name,token,nrows,ncols,end,active):
    if (nrows,ncols,end,active) not in [(r,c,e,a) for _,r,c,e,a in CASES]:raise ValueError('Closed matrix only')
    lines=bound_guard(path)+sentinel_guard(name,token)+[
        ...
        'activate object '+('boundWindow' if active else '(active window of qualitySentinel)'),
        ...
        'try',
        f'set diagTable to make new table at boundDoc with properties {{text object:diagRange,number of rows:{nrows},number of columns:{ncols}}}',
...
def run(output,source):
    ...
    report={'status':'RUNNING','scope':'constructor diagnostics only','cases':[],'checks':{},'steps':[],'confirmed':[],'source_input':str(source),'source_input_sha256':sha(source)}
    session=None
    def flush():
    ...
    try:
        ...
        for label,nrows,ncols,end,active in CASES:
            case={'label':label,'requested_dimensions':[nrows,ncols],'range':[12,end],'active_owned':active};report['cases'].append(case);flush()
            ...
            native=observe_native(session,output,report,label+'-one-constructor',commands(session._bound_path,report['sentinel_name'],report['sentinel_token'],nrows,ncols,end,active),
            ...
            case['body_unchanged']=native[0][5]==native[2][2];case['table_count_delta']=native[2][1]-native[0][4];flush()
        ...
        if all(report['checks'].values()) and len(report['cases'])==4 and report.get('sentinel_closed'):
