...
def snapshot_source(bound_path, checkpoint):
    """Read the exact file-owned document and its windows, without global UI state."""
    if not Path(bound_path).is_absolute() or type(checkpoint) is not int or checkpoint < 0:
        raise ValueError('Exact owned path and checkpoint are required')
    lines = ['if not application "Microsoft Word" is running then error "WPSC_DIAGNOSTIC_HOST_ABSENT"',
             ...
             'set diagnosticName to name of guardedDoc as text',
             f'if not ((current application\'s NSString\'s stringWithString:diagnosticName)\'s isEqualToString:{apple_string(Path(bound_path).name)}) then error "WPSC_DIAGNOSTIC_NAME_CHANGED"',
             'set diagnosticWindows to {}',
             'repeat with wi from 1 to count windows of guardedDoc',
             'set diagnosticWindow to window wi of guardedDoc',
             'set end of diagnosticWindows to {name of diagnosticWindow,id of diagnosticWindow}',
             'end repeat',
             ...
             *recovery.hash_commands('content of prefixRange as text', 'prefixHash'),
             'set nativeRows to {{"owned",posix full name of guardedDoc as text,diagnosticName,saved of guardedDoc,count fields of guardedDoc,count tables of guardedDoc,bodyHash,prefixHash,diagnosticWindows}}',
...
def validate_snapshot(rows, bound_path, expected_identity=None):
    def digest(value):
        return isinstance(value, str) and re.fullmatch('[0-9a-f]{64}', value) is not None
    if (not isinstance(rows, list) or not rows or not isinstance(rows[0], list)
            or len(rows[0]) != 9 or rows[0][:3] != ['owned', bound_path, Path(bound_path).name]
            or type(rows[0][3]) is not bool
            or any(type(value) is not int or value < 0 for value in rows[0][4:6])
            or not all(digest(value) for value in rows[0][6:8])
            or not isinstance(rows[0][8], list)
            or len(rows) != rows[0][5] + 1):
        raise ValueError('Independent owned-document acknowledgement is invalid')
    seen = set()
    for window in rows[0][8]:
        if (not isinstance(window, list) or len(window) != 2
                or not (window[0] is None or isinstance(window[0], str) and bool(window[0]))
                or not (window[1] is None or type(window[1]) is int and window[1] > 0)):
            raise ValueError('Independent window metadata is invalid')
        if tuple(window) in seen:
            raise ValueError('Independent window metadata is duplicated')
        seen.add(tuple(window))
    identity = {'document_name':rows[0][2], 'windows':rows[0][8]}
    if expected_identity is not None and identity != expected_identity:
        raise ValueError('Independent native document identity changed')
...
def independent_snapshot(output, label, bound_path, checkpoint, expected_identity=None):
    path = output/(label+'.applescript')
    path.write_text(snapshot_source(bound_path, checkpoint), encoding='utf-8')
    try:
    ...
    return validate_snapshot(json.loads(result.stdout), bound_path, expected_identity)
...
def run(output):
    ...
    try:
        ...
        report['owned_before_failure'] = independent_snapshot(output, 'owned-before-failure', session._bound_path, checkpoint)
        owned_identity = {'document_name':report['owned_before_failure'][0][2],
                          'windows':report['owned_before_failure'][0][8]}
        report['owned_binding']['native_identity'] = owned_identity
        ...
        after = independent_snapshot(output, 'owned-after-quarantine', session._bound_path, checkpoint, owned_identity)
