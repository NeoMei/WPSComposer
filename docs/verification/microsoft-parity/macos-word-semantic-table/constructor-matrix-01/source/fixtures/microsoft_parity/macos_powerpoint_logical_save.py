...
return name of sentinelDoc''',mutation=True)
        ...
        for name,expected_text in expected.items():
            ...
            with opened(path) as session:
                ...
                assert inventory(session)==before
                copy_job(session,output,'reopen-'+Path(name).stem)
        ...
        report['artifact_hashes']={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in output.glob('*.pptx') if p.is_file()}
