...
class CheckpointSnapshot:
    ...
    tracked_numbering_prefix: tuple
...
def _tracking(session):
    return (tuple(tuple(item) for item in getattr(session,'_tracked_indexes',())),
            ...
            tuple(tuple(item) for item in getattr(session,'_tracked_numbering',())))
...
def checkpoint(session):
    try:
        ...
        session._remaining()
        indexes,refs,numbering=_tracking(session)
        state=_parse(session,session._execute(state_commands(indexes+refs+numbering)))
        identities={row[1]:row[4] for row in state if row[0]=='bookmark'}
        if identities!={handle.bookmark:code for handle,code in indexes+refs+numbering}:_malformed(session)
        item=CheckpointSnapshot(state[0][2],state,indexes,refs,numbering,getattr(session,'_observed_field_topology',None))
        mapping=getattr(session,'_degradation_checkpoints',{})
        old=mapping.get(item.coordinate)
        if old is not None and (old.state,old.tracked_indexes_prefix,old.tracked_references_prefix,old.tracked_numbering_prefix)!=(item.state,item.tracked_indexes_prefix,item.tracked_references_prefix,item.tracked_numbering_prefix):raise ValueError('ambiguous checkpoint')
...
def rollback_commands(saved,current,appended):
    tracked=saved.tracked_indexes_prefix+saved.tracked_references_prefix+saved.tracked_numbering_prefix
...
def rollback(session,checkpoint):
    try:
        ...
        indexes,refs,numbering=_tracking(session)
        if indexes[:len(saved.tracked_indexes_prefix)]!=saved.tracked_indexes_prefix or refs[:len(saved.tracked_references_prefix)]!=saved.tracked_references_prefix or numbering[:len(saved.tracked_numbering_prefix)]!=saved.tracked_numbering_prefix:raise ValueError('tracking prefix changed')
        current=_parse(session,session._execute(state_commands(saved.tracked_indexes_prefix+saved.tracked_references_prefix+saved.tracked_numbering_prefix,target)),target)
        ...
        session._tracked_numbering=list(saved.tracked_numbering_prefix)
