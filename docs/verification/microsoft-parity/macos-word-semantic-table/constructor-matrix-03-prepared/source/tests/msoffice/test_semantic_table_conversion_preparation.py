...
def test_complete_ack_accepts_observations_without_promoting_to_preservation_pass():
    rows=ack_rows()
    assert mod().valid(rows,'/owned.docx',end=19)
    rows[-1][1]='changed prefix'
    assert mod().valid(rows,'/owned.docx',end=19)  # valid evidence may reveal failure
