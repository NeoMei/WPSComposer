...
set content of literalRange to "Owned "
set insertionPoint to insertionPoint + 6
set literalRange to create range boundDoc start literalStart end insertionPoint
if (content of literalRange as text) is not "Owned " then error "WPSC_REFERENCE_TEXT_UNVERIFIED"
...
set literalStart to insertionPoint
set literalRange to create range boundDoc start insertionPoint end insertionPoint
set content of literalRange to "fallback"
set insertionPoint to insertionPoint + 8
set literalRange to create range boundDoc start literalStart end insertionPoint
if (content of literalRange as text) is not "fallback" then error "WPSC_REFERENCE_TEXT_UNVERIFIED"
set end of nativeRows to {"literal",0,literalStart,insertionPoint,content of literalRange as text}
end try
if not referenceFailed and not referenceDegraded then
make new bookmark at boundDoc with properties {name:"WPSC_R_975cd0d09a7f4224b4c75ddcad1642", text object:field code of ownField}
if (content of text object of bookmark "WPSC_R_975cd0d09a7f4224b4c75ddcad1642" of boundDoc as text) is not referenceCode then error "WPSC_REFERENCE_IDENTITY_UNVERIFIED"
set end of nativeRows to {"reference",0,previousFields,count fields of boundDoc,referenceStart + 1,referenceResultStart,referenceResultEnd,referenceCode,"WPSC_R_975cd0d09a7f4224b4c75ddcad1642"}
end if
if not referenceFailed then
set literalStart to insertionPoint
set literalRange to create range boundDoc start insertionPoint end insertionPoint
set content of literalRange to " end"
set insertionPoint to insertionPoint + 4
set literalRange to create range boundDoc start literalStart end insertionPoint
if (content of literalRange as text) is not " end" then error "WPSC_REFERENCE_TEXT_UNVERIFIED"
set end of nativeRows to {"literal",0,literalStart,insertionPoint,content of literalRange as text}
end if
end if
if not referenceFailed then
