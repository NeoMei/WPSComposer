...
return name of sentinelBook''')
        ...
        for case in ('before-selected', 'selected', 'after-selected'):
            ...
            expected_names = [s['name'] for s in initial['sheets']] + [
                'DeleteProbeB' if case == 'before-selected' else 'DeleteProbeA'
            ]
            ...
            assert [s['name'] for s in snap['sheets']] == expected_names
            ...
            assert [s['name'] for s in reopened['sheets']] == expected_names
        ...
        if session is not None and not session._closed and not session._failed:
            try:
                ...
                report['passed'] = False
        if sentinel is None and sentinel_creation_started:
            # A native failure may occur after creation but before its name is
            # acknowledged. Recover only one added workbook with our marker.
            try:
                current = _inventory(output, 'inventory-after-unacknowledged-create')
                identities = {(item['name'], item['path']) for item in before}
                added = [item for item in current if (item['name'], item['path']) not in identities]
                assert len(added) == 1, 'Sentinel identity uncertain; manual reconciliation required'
                candidate = added[0]['name']
                state = _osascript(output, 'verify-unacknowledged-sentinel', f'''set sentinelBook to workbook {_quote(candidate)}
return ((saved of sentinelBook) as text) & (character id 31) & ((value of range "A1" of worksheet 1 of sentinelBook) as text)''').split(chr(31))
                assert state == ['false', marker], 'Added workbook is not the owned sentinel'
                sentinel = candidate
                report['unacknowledged_sentinel_identified'] = True
            except BaseException as error:
                report['sentinel_identity_unresolved'] = str(error)
                report['passed'] = False
...
return ((saved of sentinelBook) as text) & (character id 31) & ((value of range "A1" of worksheet 1 of sentinelBook) as text)''').split(chr(31))
                ...
                _osascript(output, 'close-exact-sentinel', f'''set sentinelBook to workbook {_quote(sentinel)}
if saved of sentinelBook then error "Sentinel saved state changed"
if (value of range "A1" of worksheet 1 of sentinelBook) is not {_quote(marker)} then error "Sentinel marker changed"
