...
class MacWordSession:
    ...
    def _retain(self, reason):
        """Quarantine in memory before attempting fallible diagnostic storage."""
        ...
        try:
            if self.lock and not self.lock.quarantine_path.exists():
                ...
                self.__dict__.pop('_quarantine_write_error', None)
        except BaseException as exc:
            # Retention is secondary to the native error/cancellation. A failed
            # write must neither replace it nor reopen the in-memory session.
            self._quarantine_write_error = type(exc).__name__
        # An existing file may be the partial output of a previous failed
        # write. Its mere existence must not clear that failure on re-retention.

    def _quarantine_location(self):
        """Advertise only an existing quarantine record, never an intended path."""
        try:
            if self.lock and not getattr(self, '_quarantine_write_error', None):
                path = self.lock.quarantine_path
                if path.is_file():
                    return path
        except OSError:
            pass
    ...
    def _write_diagnostic(self, path, text, *, preserve_error=False):
        """Return a written diagnostic path; retain failures without private text."""
        try:
            path.write_text(text, encoding='utf-8')
        except BaseException as exc:
            self._diagnostic_write_error = type(exc).__name__
            self._retain_evidence = True
            if not preserve_error and not isinstance(exc, Exception):
                raise
            return None
        return path
    ...
    def _execute(self, lines, *, bind=True, launch=True):
        ...
        try:
            ...
            diagnostic = self._write_diagnostic(
                log, type(exc).__name__ + '\n' + partial(getattr(exc, 'stdout', None)) + '\n' + partial(getattr(exc, 'stderr', None)),
                preserve_error=True,
            )
            cls = NativeWordTimeoutError if isinstance(exc, subprocess.TimeoutExpired) else NativeWordError
            kw = dict(staging_path=self.staging_root, diagnostic_path=diagnostic, quarantine_path=self._quarantine_location())
            raise (cls(**kw) if cls is NativeWordTimeoutError else cls('NATIVE_WORD_QUARANTINED', **kw)) from None
        except BaseException as exc:
            self._retain('Native AppleEvent completion uncertain')
            self._write_diagnostic(log, type(exc).__name__ + '\n', preserve_error=True)
            raise
        # Everything below follows an actual submission. Diagnostic I/O is not
        # an ACK and may not hide the process's original error classification.
        try:
            diagnostic = self._write_diagnostic(
                log, result.stderr + '\n' + result.stdout, preserve_error=bool(result.returncode),
            )
            if diagnostic is None:
                self._retain('Native diagnostic persistence failed after submission')
            if result.returncode:
                self._retain_evidence = True
                if '-1712' in result.stderr:
                    self._retain('Native AppleEvent completion uncertain')
                    raise NativeWordTimeoutError(staging_path=self.staging_root, diagnostic_path=diagnostic, quarantine_path=self._quarantine_location())
                if re.search(r'\(-609\)\s*$', result.stderr):
                    self._retain('Native AppleEvent connection invalid')
                    raise NativeWordError('NATIVE_WORD_QUARANTINED', staging_path=self.staging_root, diagnostic_path=diagnostic, quarantine_path=self._quarantine_location())
                ...
                if 'WPSC_FIELD_IDENTITY_STALE (-2700)' in result.stderr:
                    raise NativeWordError('NATIVE_WORD_FIELD_IDENTITY_STALE', staging_path=self.staging_root, diagnostic_path=diagnostic)
                raise NativeWordError('NATIVE_WORD_EXECUTION_FAILED', staging_path=self.staging_root, diagnostic_path=diagnostic)
            if diagnostic is None:
                raise NativeWordError('NATIVE_WORD_QUARANTINED', staging_path=self.staging_root, quarantine_path=self._quarantine_location())
            try:
                ...
                raise NativeWordError('NATIVE_WORD_QUARANTINED', staging_path=self.staging_root, diagnostic_path=diagnostic, quarantine_path=self._quarantine_location()) from None
        except NativeWordError:
            raise
        except BaseException:
            self._retain('Native result processing failed after submission')
            raise
    ...
    def _remaining(self):
        try:
            ...
            raise NativeWordTimeoutError(staging_path=self.staging_root, quarantine_path=self._quarantine_location() if self._quarantined else None) from None
    ...
    def reserve_document_quality_anchor(self, title="生成质量提示", notices=()):
        from .macos_word_quality import reserve_document_quality_anchor
        return reserve_document_quality_anchor(self, title, notices)

    def upsert_document_quality_notice(self, issue):
        from .macos_word_quality import upsert_document_quality_notice
        return upsert_document_quality_notice(self, issue)

    def add_document_quality_notice(self, notices):
        from .macos_word_quality import add_document_quality_notice
        return add_document_quality_notice(self, notices)

    def add_quality_notice_at_bookmark(
        self, *, code, message, fallback, node_id, page, bookmark_name=None
    ):
        from .macos_word_quality import add_quality_notice_at_bookmark
        return add_quality_notice_at_bookmark(
            self, code=code, message=message, fallback=fallback, node_id=node_id,
    ...
    def add_equation_number_native(
        ...
        from .macos_word_numbering import add_equation_number_native
        return add_equation_number_native(
            self, source=source, numbering=numbering, bookmarkName=bookmarkName,
            fallbackText=fallbackText, owner_node_id=owner_node_id,
        )
    ...
    def _add_native_caption(
        ...
        *, keep_with_next=False,
        ...
        from .macos_word_numbering import add_native_caption
        return add_native_caption(
            self, caption, numbering, bookmark_name, owner_node_id,
