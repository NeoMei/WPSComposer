...
def test_caption_fixture_rejects_body_prefix_suffix_or_paragraph_corruption():
    module=importlib.import_module('fixtures.microsoft_parity.macos_word_caption')
    rows=json.loads((module.ROOT/'build/word-caption-20260911/native-run-01/report.json').read_text())['native_rows']
...
def test_caption_fixture_validates_nondefault_format_and_trailing_paragraph():
    ...
    assert module.validate_format_readback(global_rows,'global',following=' ')
    assert module.validate_format_readback(chapter_rows,'chapter',following='\r')
    for row_index,column,value in (
        ...
        assert not module.validate_format_readback(broken,'global',following=' ')
