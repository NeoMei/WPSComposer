...
MODES=('font-properties','paragraph-object','detached-properties','list-identity','font-scalar-copy')
...
def seed_commands(mode):
    if mode=='font-scalar-copy':mode='font-properties'
...
def phases(mode,name):
    if mode=='font-scalar-copy':return [seed_commands(mode),diagnostic_readback(),scalar_copy_commands(),diagnostic_readback()]
    if mode not in MODES:raise ValueError('Unknown mode')
...
def paragraph_style_by_name(styles,name):
    """Resolve semantic paragraph-style identity; style IDs are document-local."""
    matches=[]
    for style in styles:
        names=style.findall(W+'name')
        if (style.get(W+'type')=='paragraph' and len(names)==1
                and (names[0].get(W+'val') or '').casefold()==name.casefold()):
            matches.append(style)
    if len(matches)!=1 or not matches[0].get(W+'styleId'):return None
    # Duplicate IDs would make basedOn/pStyle references ambiguous even if names
    # differed. Reject the package rather than choose the first declaration.
    identifier=matches[0].get(W+'styleId')
    if sum(s.get(W+'styleId')==identifier for s in styles)!=1:return None
    return matches[0]
...
def clone_xml_valid(xml,mode):
    ...
    if source is None or clone is None or normal is None:return False
    bases=clone.findall(W+'basedOn')
    if len(bases)!=1 or bases[0].get(W+'val')!=normal.get(W+'styleId'):return False
    ...
    return all(source.find(W+tag) is not None and canonical(source.find(W+tag))==canonical(clone.find(W+tag)) for tag in tags)
...
def list_xml_valid(styles_xml,numbering_xml):
    ...
    ids=[];style_ids=[]
    for level in range(1,5):
        selected=paragraph_style_by_name(styles.findall(W+'style'),'heading '+str(level))
        if selected is None:return False
        style_ids.append(selected.get(W+'styleId'))
        num=selected.find('./'+W+'pPr/'+W+'numPr/'+W+'numId')
    ...
    for level in range(4):
        ...
        lvl=matches[0]
        expected={'pStyle':style_ids[level],'lvlText':'.'.join('%'+str(i) for i in range(1,level+2)),'start':'1','numFmt':'decimal'}
...
def run(output,mode):
    ...
    if mode=='font-scalar-copy':report['diagnostic']={}
    try:
        ...
        with MacWordSession.new_document(visible=False) as session:
            ...
            for i,phase in enumerate(commands):
                ...
                if mode=='font-scalar-copy':
                    if i in (1,3):
                        observation='before_scalar_copy' if i==1 else 'after_scalar_copy'
                        record_observation(report,observation,rows)
                        valid=True
                        if i==3:
                            verdict=scalar_copy_verdict(report['diagnostic']['before_scalar_copy']['rows'],rows)
                            report['checks'].update(verdict)
                            valid=all(verdict.values())
                    else:valid=rows==[['stage',True]]
                elif mode=='list-identity':
                ...
                if (mode not in ('list-identity','font-scalar-copy') and i==2) or (mode=='font-scalar-copy' and i==3):session.save_docx(output/'pre-list.docx')
        ...
        with MacWordSession.open_document(output/'heading.docx',read_only=True,visible=False) as reopened:
            ...
            if mode=='font-scalar-copy':
                rows=reopened._execute(diagnostic_readback());report['reopen_scalar_rows']=rows
                record_observation(report,'reopen_scalar_copy',rows)
                verdict=scalar_copy_verdict(report['diagnostic']['before_scalar_copy']['rows'],rows)
                report['checks'].update({'reopen_'+k:v for k,v in verdict.items()})
            elif mode in ('list-identity','detached-properties'):
...
def scalar_copy_commands():
    """Try every dictionary-writable font/nested scalar; never ignore failure."""
    lines=bindings()+['set base style of detachedStyle to style normal']
    for label,left,right in dimensions('font-properties'):
        lines += [f'set scalarSourceValue to {left}',f'set scalarDestinationValue to {right}',
                  ...
                  'else','set scalarEqual to (scalarSourceValue is equal to scalarDestinationValue)','end if',
                  'if not scalarEqual then',f'set {right} to scalarSourceValue','end if']
    return lines+['set nativeRows to {{"stage",true}}']


def scalar_copy_verdict(before,after):
    if not diagnostic_rows_valid(before) or not diagnostic_rows_valid(after):
        raise AssertionError('Scalar copy observation is malformed')
    return {'source_properties_preserved':all(a[1]==b[1] for a,b in zip(before,after)),
            'complete_font_appearance_equal':all(r[3] for r in after)}
...
FORMATTED_TEXT='FORMATTED STYLE 中文 العربية'


def formatted_seed():
    return seed_commands('detached-properties')[:-1]+[
        f'set content of text object of boundDoc to {apple_string(FORMATTED_TEXT)} & return & {apple_string(FORMATTED_TEXT)} & return',
        'set style of text object of paragraph 1 of boundDoc to sourceStyle',
        'set style of text object of paragraph 2 of boundDoc to detachedStyle',
        'set nativeRows to {{"stage",true}}']


def detached_identity_guard(paragraph=2):
    return [f'set observedStyleName to name local of style of text object of paragraph {paragraph} of boundDoc as text',
            f'if not (((current application\'s NSString\'s stringWithString:observedStyleName)\'s isEqualToString:{apple_string(CLONE)}) as boolean) then error "WPSC_FORMATTED_STYLE_IDENTITY"']


def formatted_transfer_commands():
    return bindings()+detached_identity_guard()+[
...
def formatted_fresh_paragraph():
    return [f'set detachedStyle to Word style {apple_string(CLONE)} of boundDoc',
        'set p to (end of content of text object of boundDoc) - 1',
        'set newRange to create range boundDoc start p end p',
        f'set content of newRange to {apple_string(FORMATTED_TEXT)} & return',
        'set newRange to text object of paragraph 3 of boundDoc',
        'set style of newRange to detachedStyle','reset font object of newRange','reset paragraph format of newRange',
        *detached_identity_guard(3),'set nativeRows to {{"stage",true}}']


def formatted_fresh_readback():
    # Font comparison of a fresh style-only paragraph against the custom style.
    return [line.replace('set sourceFont to font object of sourceStyle',
                        'set sourceFont to font object of text object of paragraph 3 of boundDoc')
            for line in diagnostic_readback()]


def formatted_source_change(size,spacing):
    from skills.WPSComposer.scripts.msoffice.macos_word_session import _number
    return ['set sourceStyle to Word style (style heading1) of boundDoc',
            f'set font size of font object of sourceStyle to {_number(size)}',
            f'set space before of paragraph format of sourceStyle to {_number(spacing)}',
            'set nativeRows to {{"stage",true}}']


def formatted_phases():
    return [formatted_seed(),formatted_transfer_commands(),formatted_fresh_paragraph(),
            diagnostic_readback(),clone_readback('detached-properties'),formatted_fresh_readback(),
            formatted_source_change(28,27),formatted_source_change(17,14)]


def style_definition(xml,name):
    style=paragraph_style_by_name(ET.fromstring(xml).findall(W+'style'),name)
    if style is None:return None
    # Ignore only revision bookkeeping; full style identity, base and appearance
    # children (including rPr/pPr, szCs and linked-style references) remain exact.
    return (tuple(sorted(style.attrib.items())),tuple(canonical(child) for child in style if child.tag!=W+'rsid'))


def package_styles(path):
    with ZipFile(path) as z:return z.read('word/styles.xml')


def run_formatted_candidate(output):
    report={'status':'FAIL','mode':'formatted-style-copy','checks':{},'diagnostic':{},
            'scope':'experimental native style redefinition; complete named-style equality and independence required',
            'source_hashes':retain_sources(output,SOURCES)}
    session=None;reopened=None;sentinel_name=None;token=None;preimage_hash=None
    def flush():(output/'report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
    def stage(label,commands):
        report['current_phase']=label;flush()
        rows=session._execute(commands);report.setdefault('stage_rows',{})[label]=rows;flush()
        if rows!=[['stage',True]]:raise AssertionError('Formatted stage acknowledgement invalid: '+label)
        report['checks'][label]=True
    def observation(owner,label,commands=None):
        rows=owner._execute(diagnostic_readback() if commands is None else commands)
        report.setdefault('raw_rows',{})[label]=rows;flush()
        record_observation(report,label,rows);flush();return rows
    def check(label,value):
        report['checks'][label]=bool(value);flush()
        if not value:raise AssertionError('Formatted style check failed: '+label)
    try:
        report['dictionary_sha256']=sha('/Applications/Microsoft Word.app/Contents/Resources/Word.sdef')
        report['inventory_before']=inventory(output,'before')
        with MacWordSession.new_document(visible=False) as session:
            session._retain_evidence=True
            token='FORMATTED STYLE SENTINEL '+uuid4().hex+' 中文😀'
            rows=session._execute(['set sentinelDoc to make new document',f'set content of text object of sentinelDoc to {apple_string(token)}','set nativeRows to {{name of sentinelDoc as text}}'])
            if len(rows)!=1 or len(rows[0])!=1 or not isinstance(rows[0][0],str):raise RuntimeError('Sentinel creation ACK invalid')
            sentinel_name=rows[0][0];report['sentinel_name']=sentinel_name
            report['sentinel_before']=sentinel_preimage(inventory(output,'sentinel-before'),sentinel_name,token)
            report['word_version']=session._execute(['set nativeRows to {{version as text}}'])
            stage('seed',formatted_seed())
            before=observation(session,'before_copy')
            source_size=next(r[1][1] for r in before if r[0]=='font:font size')
            spacing_rows=session._execute(['set nativeRows to {{space before of paragraph format of Word style (style heading1) of boundDoc}}'])
            if len(spacing_rows)!=1 or len(spacing_rows[0])!=1 or type(spacing_rows[0][0]) not in (int,float):raise AssertionError('Source spacing observation invalid')
            source_spacing=spacing_rows[0][0]
            session.save_docx(output/'source-preimage.docx');preimage_hash=sha(output/'source-preimage.docx')
            preimage_styles=package_styles(output/'source-preimage.docx')
            source_definition=style_definition(preimage_styles,'heading 1')
            check('source_definition_unique',source_definition is not None)
            stage('formatted_text_style_redefinition',formatted_transfer_commands())
            after=observation(session,'after_copy')
            for key,value in scalar_copy_verdict(before,after).items():check(key,value)
            paragraph_rows=session._execute(clone_readback('detached-properties'));report['full_native_rows']=paragraph_rows
            check('full_font_paragraph_native_equal',native_valid('detached-properties',paragraph_rows))
            stage('fresh_style_only_paragraph',formatted_fresh_paragraph())
            fresh=observation(session,'fresh_style_only',formatted_fresh_readback())
            check('fresh_paragraph_font_equal',all(r[3] for r in fresh))
            session.save_docx(output/'before-source-challenge.docx')
            styles=package_styles(output/'before-source-challenge.docx')
            (output/'before-source-challenge-styles.xml').write_bytes(styles)
            check('source_definition_unchanged_by_copy',style_definition(styles,'heading 1')==source_definition)
            check('complete_font_paragraph_xml_equal',clone_xml_valid(styles,'detached-properties'))
            clone_definition=style_definition(styles,CLONE)
            # Source is changed only as a private independence challenge, then restored.
            stage('source_independence_challenge',formatted_source_change(source_size+11,source_spacing+13))
            challenged=observation(session,'after_source_challenge')
            check('clone_native_unchanged_after_source_change',all(a[2]==b[2] for a,b in zip(after,challenged)))
            fresh=observation(session,'fresh_after_source_challenge',formatted_fresh_readback())
            check('fresh_paragraph_independent',all(r[3] for r in fresh))
            session.save_docx(output/'source-challenged.docx')
            check('clone_definition_independent',style_definition(package_styles(output/'source-challenged.docx'),CLONE)==clone_definition)
            stage('restore_private_source',formatted_source_change(source_size,source_spacing))
            restored=observation(session,'after_source_restore')
            for key,value in scalar_copy_verdict(before,restored).items():check('restored_'+key,value)
            session.save_docx(output/'heading.docx');session.export_pdf(output/'heading.pdf')
            final_styles=package_styles(output/'heading.docx');(output/'styles.xml').write_bytes(final_styles)
            check('source_definition_restored',style_definition(final_styles,'heading 1')==source_definition)
            check('clone_definition_preserved',style_definition(final_styles,CLONE)==clone_definition)
        close_after_owned(session,output,report,sentinel_name,token);sentinel_name=None
        digest=sha(output/'heading.docx')
        with MacWordSession.open_document(output/'heading.docx',read_only=True,visible=False) as reopened:
            reopened._retain_evidence=True
            rows=observation(reopened,'reopen_copy')
            for key,value in scalar_copy_verdict(before,rows).items():check('reopen_'+key,value)
            rows=reopened._execute(clone_readback('detached-properties'));report['reopen_full_native_rows']=rows
            check('reopen_full_font_paragraph_equal',native_valid('detached-properties',rows))
            rows=observation(reopened,'reopen_fresh',formatted_fresh_readback());check('reopen_fresh_paragraph_equal',all(r[3] for r in rows))
        check('reopen_source_bytes_preserved',digest==sha(output/'heading.docx'))
        check('preimage_file_preserved',preimage_hash==sha(output/'source-preimage.docx'))
        report['inventory_final']=inventory(output,'final');check('final_inventory_preserved',report['inventory_final']==report['inventory_before'])
        import fitz
        with fitz.open(output/'heading.pdf') as pdf:
            text='\n'.join(p.get_text() for p in pdf)
            for i,p in enumerate(pdf):p.get_pixmap(matrix=fitz.Matrix(1.5,1.5)).save(output/('page-%s.png'%(i+1)))
        (output/'pdf-text.txt').write_text(text)
        check('pdf_three_paragraphs_visible',text.count('FORMATTED STYLE')==3)
        check('sources_unchanged',all(sha(ROOT/p)==v for p,v in report['source_hashes'].items()))
        report['status']='PASS' if all(report['checks'].values()) else 'FAIL'
    except BaseException as exc:
...
def main(argv=None):
    ...
    parser.add_argument('--mode',choices=MODES+('font-order-diagnostic','formatted-style-copy'),required=True);parser.add_argument('--output',type=Path,required=True)
    args=parser.parse_args(argv)
    if not args.execute:
        print('Native feasibility requires explicit --execute',file=sys.stderr);return 2
    output=args.output.resolve();output.mkdir(parents=True,exist_ok=False)
    if args.mode=='formatted-style-copy':
        return 0 if run_formatted_candidate(output)['status']=='PASS' else 1
