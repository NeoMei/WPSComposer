...
def inspect_native_styles(session, rows):
    ...
    for row in rows:
        ...
        if text.startswith('Static '):
            for offset,extent,want_styled in [(0,8,False),(8,13,True),(21,1,False)]:
                commands += [f'set r to create range boundDoc start {start+offset} end {start+offset+extent}',
                ...
                expected.append(['static-style',want_styled,want_styled,want_styled])
        cursor=0
        while True:
