    ...
    try:
        ...
        with create_document('writer',engine='msoffice',visible=False) as session:
            try:
                ...
                session.add_heading_level_native(text='Chapter',level=1)
                ...
                if sentinel and not session._quarantined:
                    journal.add('sentinel-close',session._execute([f'set d to document {apple_string(sentinel)}',f'if (content of text object of d as text) is not {apple_string(sentinel_token)} & return then error "SENTINEL_CHANGED"','if saved of d then error "SENTINEL_SAVED"','close d saving no','set nativeRows to {{"closed"}}']))
                    sentinel=None
                if session.staging_root.exists():shutil.copytree(session.staging_root,output/'native-runtime',dirs_exist_ok=True)
        assert journal.add('after-owned-close',inventory(output,'after-owned-close'))==starting
