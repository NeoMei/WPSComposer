...
def _literal(text, index, *, style=False):
    ...
    if style:
        lines += ['set italic of font object of literalRange to true',
                  ...
                  f'set end of nativeRows to {{"styled",{index},literalStart,insertionPoint,italic of font object of literalRange,(color of font object of literalRange is {{40092, 0, 1542}}),(background pattern color of shading of literalRange is {{64764, 59624, 59110}})}}']
    return lines
...
def paragraph_commands(session, runs, indent, controller_owned, static):
    lines = _start(session) + ['set referenceFailed to false']
    handles = {}
...
def paragraph(session, runs, owner, list_formatting, controller_owned=False, static=False):
    ...
    pending=list(rows)
    position=None;start=None;tracked=[];degraded=False;literal_paragraphs=0
    def take():
    ...
    def literal(text,index,styled=False):
        ...
        if styled and take()!=['styled',index,position,end,True,True,True]:_invalid(session)
        position=end
